  ...
  default: ({ register, errors, setValue, trigger, brand }: any) => (
    <div data-testid="brand-general-info">
      <input
        ...
        {...register('name', { required: true, onChange: vi.fn() })}
        placeholder="Name"
      />
      <input
        data-testid="input-slug"
        defaultValue={brand?.slug}
        {...register('slug', { required: true, onChange: vi.fn() })}
        placeholder="Slug"
      />
      <input
        type="checkbox"
...
describe('BrandEdit Page', () => {
  ...
  describe('getBrand', () => {
    ...
    it('should not call getBrand when id is missing', async () => {
      (useRouter as any).mockReturnValue({
        push: mockRouterPush,
        replace: mockRouterReplace,
        query: {},
  ...
  describe('Form Submission', () => {
    ...
    it('should not submit when id is missing', async () => {
      (useRouter as any).mockReturnValue({
        push: mockRouterPush,
        replace: mockRouterReplace,
        query: {},
      ...
      expect(editBrand).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle brand with empty name and slug', async () => {
      const emptyBrand = { id: 1, name: '', slug: '', isPublish: false };
      (getBrand as any).mockResolvedValue(emptyBrand);
      render(<BrandEdit />);
      ...
      await waitFor(() => {
        ...
        expect(slugInput.defaultValue).toBe('');
      });
    });
  });
});