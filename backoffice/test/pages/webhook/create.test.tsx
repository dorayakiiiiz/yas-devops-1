...
describe('WebhookCreate Page', () => {
  ...
  describe('createWebhook', () => {
    ...
    it('should be called with correct payload', async () => {
      ...
      (createWebhook as any).mockResolvedValue({ status: 201 });
      render(<WebhookCreate />);
      
      // The create function should be available
      expect(createWebhook).toBeDefined();
    });

    it('should handle API error', async () => {
      const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
      (createWebhook as any).mockRejectedValue(new Error('Create failed'));
      render(<WebhookCreate />);
      await waitFor(() => {
